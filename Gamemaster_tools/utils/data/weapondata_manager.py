...
class WeaponDataManager:
    ...
    def __init__(self, json_path=None):
        if json_path is None:
            json_path = str(WEAPONS_SHIELDS_JSON)
        self._json_path = os.path.abspath(json_path)

    # JSON helpers (replace JsonManager usage)
    def load(self):
        return load_json_safe(self._json_path, default=[])

    def save(self, items):
        save_json(self._json_path, items)
    ...
    def build_item_from_fields(self, fields):
        ...
        try:
            ...
            logger.warning(f"Failed to calculate price from fields, using copper only: {e}")
