...
class PlaceholderManager:
    ...
    def add_resolution(self, placeholder_id: str, target_skill_id: str, category: str = None, notes: str = None):
    ...
    def is_placeholder(self, skill_id: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            result = conn.execute(
                "SELECT placeholder FROM skills WHERE id = ?",
                (skill_id,)
            ).fetchone()
    ...
    for item in summary:
        ...
        if item['resolutions']:
            for res in item['resolutions'][:5]:  # Show first 5
                display = f"{res['skill_name']} ({res['parameter']})" if res['parameter'] else res['skill_name']
