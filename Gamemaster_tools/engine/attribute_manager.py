...
class AttributeManager:
    ...
    def __init__(self, class_db: ClassDBManager):
        ...
        self.original_class_values: dict[str, int] = (
            {}
    ...
    def initialize_for_class(self, class_name: str, race: str, age: int) -> None:
        ...
        # Store race and age modifiers using RaceManager
        race_id = self._normalize_race_name(race)
        race_obj = _race_manager.get_race(race_id)

        if race_obj:
            self.race_modifiers = race_obj.attributes.modifiers.model_dump()
            age_category = race_obj.get_age_category(age)
            self.age_modifiers = age_category.modifiers.model_dump() if age_category else {}
        else:
            self.race_modifiers = {}
            self.age_modifiers = {}
        ...
        for attr in self.ATTRIBUTES:
            ...
            if attr in self.double_roll_stats:
                ...
                self.class_values[attr] = max(
                    normal_like_roll(min_val, max_val), normal_like_roll(min_val, max_val)
                )
            else:
    ...
    def get_hybrid_spent_and_generated(self) -> tuple[int, int]:
        ...
        for attr in self.ATTRIBUTES:
            ...
            base_cost = self._calculate_point_cost(
                attr, self._get_attr_value(attr, use_original=True)
            )
    ...
    def _apply_modifiers(self, race: str, age: int) -> None:
        ...
        temp_stats = dict(self.class_values)

        # Apply age modifiers
        if race_obj:
            age_category = race_obj.get_age_category(age)
            if age_category and age_category.modifiers:
                for attr, mod in age_category.modifiers.model_dump().items():
                    if attr in temp_stats:
                        temp_stats[attr] += mod

        # Apply race modifiers
        if race_obj:
            for attr, mod in race_obj.attributes.modifiers.model_dump().items():
                if attr in temp_stats:
                    temp_stats[attr] += mod
