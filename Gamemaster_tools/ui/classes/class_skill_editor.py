...
class SkillAssignDialog(QtWidgets.QDialog):
    ...
    def on_ok(self):
        ...
        if self.skill_type == 1:  # Level-based
            skill_percent = None
            if skill_level == 0:
                QtWidgets.QMessageBox.critical(self, "Hiba", "A szint alapú képzettségnél add meg a szintet!")
                return
        elif self.skill_type == 2:  # Percentage-based
            skill_level = None
            if skill_percent == 0:
                QtWidgets.QMessageBox.critical(self, "Hiba", "A % alapú képzettségnél add meg a százalékot!")
                return
        else:
            # No type info - manual validation
            if skill_level and skill_percent:
                QtWidgets.QMessageBox.critical(self, "Hiba", "Csak az egyik mezőt töltsd ki: szint vagy százalék!")
                return
...
class ClassSkillEditorWidget(QtWidgets.QWidget):
    ...
    def _init_ui(self):
        ...
        self.assigned_skills_table.setHorizontalHeaderLabels(["Azonosító", "Név", "Szint", "Képzettség szint", "Képzettség %"])
        self.assigned_skills_table.horizontalHeader().setSectionResizeMode(0, QtWidgets.QHeaderView.ResizeToContents)
        self.assigned_skills_table.horizontalHeader().setSectionResizeMode(1, QtWidgets.QHeaderView.Stretch)
        self.assigned_skills_table.horizontalHeader().setSectionResizeMode(2, QtWidgets.QHeaderView.ResizeToContents)
        self.assigned_skills_table.horizontalHeader().setSectionResizeMode(3, QtWidgets.QHeaderView.ResizeToContents)
        self.assigned_skills_table.horizontalHeader().setSectionResizeMode(4, QtWidgets.QHeaderView.ResizeToContents)
    ...
    def populate_assigned_skills(self):
        ...
        for skill_id, class_level, skill_level, skill_percent in records:
            ...
            self.assigned_skills_table.setItem(row, 2, QtWidgets.QTableWidgetItem(str(class_level if class_level is not None else "")))
            self.assigned_skills_table.setItem(row, 3, QtWidgets.QTableWidgetItem(str(skill_level if skill_level is not None else "")))
            self.assigned_skills_table.setItem(row, 4, QtWidgets.QTableWidgetItem(str(skill_percent if skill_percent is not None else "")))
        ...
        try:
            with sqlite3.connect(DB_CLASS) as conn:
                conn.execute(
                    ...
                    (self.current_class_id, self.current_spec_id, class_level, skill_id, skill_level, skill_percent)
                )
                conn.commit()
            ...
            with sqlite3.connect(DB_CLASS) as conn:
                if self.current_spec_id is None:
                    ...
                    conn.execute(
                        ...
                        (class_level, skill_level, skill_percent, self.current_class_id, self.current_spec_id, skill_id)
                    )
                conn.commit()
        except Exception as e:
    ...
    def add_skill(self):
        ...
        if not skill_id:
            QtWidgets.QMessageBox.warning(self, "Nincs kiválasztva", "Válassz ki egy képzettséget (nem kategóriát)!")
            return
    ...
    def edit_skill(self):
        ...
        if row < 0:
            QtWidgets.QMessageBox.warning(self, "Nincs kiválasztva", "Válassz ki egy hozzárendelt képzettséget!")
            return
