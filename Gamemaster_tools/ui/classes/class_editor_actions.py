...
class ClassEditorActions:
    ...
    def load_details(self):
        ...
        if is_spec:
            ...
            row = next((s for s in specs if s["specialisation_id"] == self.parent.current_spec_id), None)
            ...
            tabs.spec_desc_edit.setText(row["specialisation_description"] if row and row.get("specialisation_description") else generated)
        else:
        ...
        with self.parent.class_db.get_connection() as conn:
            ...
            # Load row
            c.execute("SELECT * FROM combat_stats WHERE class_id = ?", (self.parent.current_class_id,))
            row = c.fetchone()
    ...
    def save_current(self):
        ...
        with self.parent.class_db.get_connection() as conn:
            ...
            for row in range(tabs.stats_table.rowCount()):
                ...
                double_chance = 1 if tabs.stats_table.item(row, 3).checkState() == QtCore.Qt.CheckState.Checked else 0
                c.execute(
        ...
        if self.parent.current_spec_id is not None:
            ...
            self.parent.class_db.update_class_description_file(
                self.parent.current_class_id,
                tabs.spec_desc_edit.text().strip() or None
            )
    ...
    def get_description_full_path(self):
        ...
        if not desc_file:
            return None
        # __file__ is at Gamemaster_tools/ui/classes/class_editor_actions.py
        # Go up 2 levels: ui/classes -> ui -> Gamemaster_tools
        ...
        if os.path.exists(path):
            try:
                ...
                QtWidgets.QMessageBox.critical(
                    self.parent, "Hiba",
                    f"Nem sikerült beolvasni a leírást:\n{e}"
                )
        else:
            tabs.desc_text_editor.clear()
    ...
    def save_description_file(self):
        ...
        try:
            ...
            QtWidgets.QMessageBox.information(
                self.parent, "Siker",
                f"Leírás elmentve: {self.get_description_filename()}"
            )
        except Exception as e:
            QtWidgets.QMessageBox.critical(
                self.parent, "Hiba",
                f"Nem sikerült menteni a leírást:\n{e}"
            )
