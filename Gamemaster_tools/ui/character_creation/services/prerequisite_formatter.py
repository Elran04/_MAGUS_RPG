...
class PrerequisiteFormatter:
    """
    Centralizes prerequisite text building and validation checks for skills.
    Wraps DB access through the provided skill_db_service and uses prereq_checker for actual validation.
    """

    def __init__(self, skill_db_service: Any, prereq_checker: Any) -> None:
    ...
    def get_prerequisite_info(
        ...
        """
        try:
            with sqlite3.connect(self.skill_db_service.get_db_path("skill")) as conn:
                rows = conn.execute(
                    """
    ...
    def get_missing_prereq_tooltip_for_level(
        ...
        skill_id: str,
        target_level: int,
        current_skills: dict[str, dict[str, int]] | None = None,
                    ...
                    if current_skills is not None:
                        ...
                        if have:
                            ...
                            if req_skill_type == 1:  # Level-based
                                ...
                                parts.append(
                                    f"{disp} - Szükséges: {required}. szint, Van: {have_level}. szint"
                                )
                            else:  # Percent-based
                                if have_percent >= required:
                                    continue  # This prereq is met, don't show it
                                parts.append(
                                    f"{disp} - Szükséges: {required}%, Van: {have_percent}%"
                                )
                        else:
