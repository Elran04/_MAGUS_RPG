...
def apply_damage(defender: Unit, damage: int, is_critical: bool = False, is_overpower: bool = False) -> dict:
    ...
    Returns:
        dict with damage breakdown: {'fp_damage': int, 'ep_damage': int}
    """
    fp_damage = 0
    ep_damage = 0
    ...
    if is_critical:
        ep_damage = damage + 3
        defender.current_ep = max(0, defender.current_ep - ep_damage)
    ...
    # Normal damage: FP first, then overflow to ÉP
        ...
        if defender.current_fp > 0:
            if damage <= defender.current_fp:
                ...
                ep_damage = damage - defender.current_fp
...
def execute_attack(state: GameState, attacker: Unit, defender: Unit) -> Tuple[bool, str]:
    ...
    if result.is_critical:
        msg = f"{attacker.name} CRITICAL HIT (100)! Dealt {result.damage + 3} ÉP damage to {defender.name}!"
    elif result.is_overpower:
        msg = f"{attacker.name} OVERPOWER STRIKE! (Total {result.total_attack} vs VÉ {defender.VE}) Dealt {result.damage} ÉP damage to {defender.name}!"
    else:
...
def handle_attack_click(state: GameState, q: int, r: int) -> bool:
    ...
    On success, advance turn and refresh reachable. Returns True if attacked.
    """
