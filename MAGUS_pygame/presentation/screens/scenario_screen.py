...
from config import BACKGROUND_SPRITES_DIR, CHARACTER_SPRITES_DIR, CHARACTERS_DIR, SCENARIOS_DIR
...
class ScenarioScreen:
    ...
    def _setup_ui_components(self) -> None:
        ...
        self.map_preview = MapPreview(x=self.screen_width - 360, y=120, width=320, height=400)

        # Character preview (right side)
        self.character_preview = CharacterPreview(
            x=self.screen_width - 340, y=120, width=300, height=460
        )

        # Roster panel (bottom)
        self.roster_panel = RosterPanel(
            x=40, y=self.screen_height - 230, width=self.screen_width - 80, height=110
        )
    ...
    def _move_selection(self, direction: int) -> None:
        ...
        if self.phase == SelectionPhase.MAP:
            self.selected_map_index = (self.selected_map_index + direction) % len(
                self.available_scenarios
            )
        elif self.phase in [SelectionPhase.TEAM_A, SelectionPhase.TEAM_B]:
            if self.available_characters:
                self.current_char_index = (self.current_char_index + direction) % len(
    ...
    def _cycle_sprite(self, direction: int) -> None:
        ...
        if self.phase in [SelectionPhase.TEAM_A, SelectionPhase.TEAM_B]:
            if self.available_sprites:
                self.current_sprite_index = (self.current_sprite_index + direction) % len(
    ...
    def _handle_next(self) -> None:
        ...
        if self.phase == SelectionPhase.MAP:
            ...
            if len(self.team_b_roster) > 0:
                ...
                logger.info(
                    f"Team B confirmed: {len(self.team_b_roster)} units - Scenario complete"
                )
    ...
    def _find_background_for_scenario(self, scenario_name: str) -> str | None:
    ...
    def _draw_team_setup(self, surface: pygame.Surface, is_team_a: bool) -> None:
        ...
        hint = self.font_small.render(
            "Character preview & roster shown →", True, self.color_instructions
        )
    ...
    def _draw_instructions(self, surface: pygame.Surface) -> None:
        ...
        if self.phase == SelectionPhase.MAP:
            instructions = ["↑↓: Select Map  |  Enter/Next: Confirm  |  ESC: Cancel"]
        else:
            instructions = [
