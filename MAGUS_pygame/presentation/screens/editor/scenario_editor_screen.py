...
from infrastructure.rendering.hex_grid import pixel_to_hex, get_grid_bounds
...
class ScenarioEditorScreen:
    ...
    def _load_scenario(self, scenario_stem: str) -> None:
        ...
        if data:
            ...
            if self.event_bus:
                try:
                    from infrastructure.events.editor_events import EditorEvent, EV_STATE_UPDATE
                    self.event_bus.publish_to_ui(EditorEvent(EV_STATE_UPDATE, {
                        ...
                        "counts": {
                            "team_a": len(self.scenario_data.get('spawn_zones', {}).get('team_a', [])),
                            "team_b": len(self.scenario_data.get('spawn_zones', {}).get('team_b', [])),
                ...
                if evt.type == EV_TOOL_SELECT:
                    ...
                    if new_name and self.scenario_data:
                        ...
                        if self.event_bus:
                            try:
                                from infrastructure.events.editor_events import EditorEvent, EV_STATE_UPDATE
                                self.event_bus.publish_to_ui(EditorEvent(EV_STATE_UPDATE, {"scenario_name": new_name}))
                            except Exception:
                                pass
                    ...
                    if self.scenario_data:
                        ...
                        if self.event_bus:
                            try:
                                from infrastructure.events.editor_events import EditorEvent, EV_STATE_UPDATE
                                self.event_bus.publish_to_ui(EditorEvent(EV_STATE_UPDATE, {"description": new_desc}))
                            except Exception:
                                pass
                    ...
                    if self.scenario_data is not None:
                        ...
                        if self.event_bus:
                            try:
                                from infrastructure.events.editor_events import EditorEvent, EV_STATE_UPDATE
                                self.event_bus.publish_to_ui(EditorEvent(EV_STATE_UPDATE, {"background": self.scenario_data.get('background')}))
                            except Exception:
                                pass
            except Exception as e:
    ...
    def _draw_editor(self, surface: pygame.Surface) -> None:
        ...
        if not self.scenario_data:
            ...
            prompt = self.font_normal.render("Use external tool: New or Load", True, self.color_text)
            surface.blit(prompt, prompt.get_rect(center=(self.screen_width // 2, 180)))
            inst = self.font_small.render("ESC: Exit", True, (180, 180, 200))
            surface.blit(inst, inst.get_rect(center=(self.screen_width // 2, self.screen_height - 40)))
            return
        ...
        tool_text = self.font_small.render(f"Tool: {self.current_tool.value}", True, self.color_text)
        ...
        inst = self.font_small.render("Click hexes to edit | Use external panel for actions | ESC to exit", True, (180, 180, 200))
