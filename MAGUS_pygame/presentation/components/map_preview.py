...
class MapPreview:
    ...
    def draw(
        ...
        if background_surface:
            ...
            scaled_bg = self._scale_background(background_surface, self.preview_width, self.preview_height)
            surface.blit(scaled_bg, (img_x, img_y))
        else:
            # Draw placeholder grid
            self._draw_placeholder_grid(surface, img_x, img_y, self.preview_width, self.preview_height)
            
            # Warning message
            warn = self.font_label.render("No background image", True, self.color_missing)
            warn_rect = warn.get_rect(center=(img_x + self.preview_width // 2, img_y + self.preview_height // 2))
    ...
    def _scale_background(self, bg_surface: pygame.Surface, max_width: int, max_height: int) -> pygame.Surface:
    ...
    def _draw_placeholder_grid(self, surface: pygame.Surface, x: int, y: int, width: int, height: int) -> None:
