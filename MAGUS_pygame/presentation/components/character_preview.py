...
class CharacterPreview:
    ...
    def _draw_combat_stats(
        self, surface: pygame.Surface, combat: dict[str, Any], x: int, y: int
    ) -> int:
    ...
    def _draw_attributes(
        self, surface: pygame.Surface, attributes: dict[str, Any], x: int, y: int
    ) -> int:
    ...
    def _draw_equipment(self, surface: pygame.Surface, equipment: Any, x: int, y: int) -> int:
        ...
        if len(equipment_items) > 3:
            more = self.font_value.render(
                f"... +{len(equipment_items) - 3} more", True, self.color_value
            )
