...
class ScenarioListDialog:
    ...
    def __init__(self, screen_width: int, screen_height: int, scenarios: list[str]):
        ...
        self.dialog_rect = pygame.Rect(
            (screen_width - dialog_w) // 2, (screen_height - dialog_h) // 2, dialog_w, dialog_h
        )
        ...
        self.btn_load = pygame.Rect(
            self.dialog_rect.centerx - 160, self.dialog_rect.bottom - 60, 140, 45
        )

        self.btn_cancel = pygame.Rect(
            self.dialog_rect.centerx + 20, self.dialog_rect.bottom - 60, 140, 45
        )
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        surface.blit(
            title, title.get_rect(center=(self.dialog_rect.centerx, self.dialog_rect.y + 40))
        )
        ...
        inst = self.font_small.render(
            "↑↓ or Click to select | Enter or Load button | ESC to cancel", True, (200, 200, 220)
        )
        surface.blit(
            inst, inst.get_rect(center=(self.dialog_rect.centerx, self.dialog_rect.bottom + 35))
        )
    ...
    def _draw_scenario_list(self, surface: pygame.Surface) -> None:
        ...
        for i in range(visible_count):
            ...
            if scenario_idx == self.selected_index:
                highlight_rect = pygame.Rect(self.list_rect.x, y, self.list_rect.width, 38)
    ...
    def _draw_buttons(self, surface: pygame.Surface) -> None:
        ...
        # Load button
        load_color = (
            self.color_button_hover if self.btn_load.collidepoint(mouse_pos) else self.color_button
        )
