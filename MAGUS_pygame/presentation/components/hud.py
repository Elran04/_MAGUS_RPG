...
class HUD:
    ...
    def draw(self, surface: pygame.Surface, unit: Optional[Unit] = None, round_num: int = 1) -> None:
        """Draw the HUD.
        ...
        self._draw_character_panel(surface, unit)
    ...
    def _draw_character_panel(self, surface: pygame.Surface, unit: Unit) -> None:
    ...
    def _draw_resource_bars(self, surface: pygame.Surface, unit: Unit) -> None:
        ...
        self._draw_bar(
            surface, x, y,
            unit.ep.current, unit.ep.maximum,
            self.color_health, "ÉP (Health)"
        )
        
        # FP bar (Fatigue/Fájdalom Pont)
        self._draw_bar(
            surface, x, y + 30,
            unit.fp.current, unit.fp.maximum,
            self.color_fatigue, "FP (Fatigue)"
        )
        
        # Stamina bar (if using stamina system - calculated from endurance)
        if hasattr(unit.attributes, 'endurance'):
            stamina = Stamina.from_attribute(unit.attributes.endurance)
            # Use FP as current stamina proxy until Unit has native stamina
            stamina.current_stamina = unit.fp.current
            percentage, state = stamina.get_state()
            
            self._draw_bar(
                surface, x, y + 60,
                stamina.current_stamina, stamina.max_stamina,
                self.color_stamina, f"Stamina ({state.value})"
            )
