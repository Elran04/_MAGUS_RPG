...
from systems.damage_calculator import calculate_damage
...
def roll_attack(attacker: Unit, defender: Unit, state: GameState = None) -> AttackResult:
    ...
    if d100 == 1:
        return AttackResult(
            hit=False, is_critical_fail=True, attack_roll=d100, total_attack=total_attack
        )
    ...
    # Miss
    return AttackResult(hit=False, attack_roll=d100, total_attack=total_attack)
...
def apply_damage(
    ...
    return {"fp_damage": fp_damage, "ep_damage": ep_damage, "max_damage_bonus": max_damage_bonus}
...
def execute_attack(state: GameState, attacker: Unit, defender: Unit) -> tuple[bool, str]:
    ...
    if not result.hit:
        return (
            True,
            f"{attacker.name} missed! (Rolled {result.attack_roll}, total {result.total_attack} vs VÉ {defender.VE})",
        )

    # Hit - apply damage
    damage_breakdown = apply_damage(
        attacker, defender, result.damage, result.is_critical, result.is_overpower
    )
...
def handle_attack_click(state: GameState, q: int, r: int) -> bool:
    ...
    """
    from actions.movement import compute_reachable
    ...
    if (q, r) == (eq, er):
        ...
        if state.active_unit.current_action_points < attack_ap_cost:
            print(
                f"{state.active_unit.name} doesn't have enough AP to attack! (Need {attack_ap_cost}, have {state.active_unit.current_action_points})"
            )
            return False
        ...
        if success:
            ...
            print(
                f"{state.active_unit.name} AP remaining: {state.active_unit.current_action_points}/{state.active_unit.max_action_points}"
            )
