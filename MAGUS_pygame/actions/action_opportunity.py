...
def handle_opportunity_attack(state: GameState, mover: Unit, triggers_opportunity: bool = None) -> Tuple[bool, str]:
    """
    ...
    print(f"[OPP ATTACK] Checking opportunity attack. Enemy: {enemy.name}, has_used: {enemy.has_used_opportunity_attack}, triggers: {triggers_opportunity}")
    ...
    if triggers_opportunity is False:
        print(f"[OPP ATTACK] No trigger - path doesn't pass through zone")
        return (False, "")
    ...
    if triggers_opportunity is None:
        ...
        if mover_pos not in enemy_reach:
            print(f"[OPP ATTACK] Mover not in enemy reach (fallback check)")
            return (False, "")
    ...
    print(f"[OPP ATTACK] No message generated")
    return (False, "")
