...
from actions.action_movement import find_path, path_passes_through_zone
from actions.action_opportunity import handle_opportunity_attack
from actions.action_attack import roll_attack, apply_damage
...
def compute_charge_targets(state: GameState) -> Set[Tuple[int, int]]:
    ...
    if distance_to_enemy >= CHARGE_MIN_DISTANCE:
        ...
        print(f"[CHARGE] {enemy_unit.name} too close to charge (distance: {distance_to_enemy}, need >= {CHARGE_MIN_DISTANCE})")
...
def execute_charge_attack(state: GameState, target_q: int, target_r: int) -> bool:
    ...
    if unit.current_action_points < CHARGE_AP_COST:
        print(f"[CHARGE] {unit.name} doesn't have enough AP (need {CHARGE_AP_COST}, have {unit.current_action_points})")
        return False
    ...
    adjacent_hexes = [
        (enemy_q + 1, enemy_r), (enemy_q + 1, enemy_r - 1), (enemy_q, enemy_r - 1),
        (enemy_q - 1, enemy_r), (enemy_q - 1, enemy_r + 1), (enemy_q, enemy_r + 1)
    ]
    ...
    if triggers_opportunity and intersection_index is not None:
        print(f"[CHARGE] Charge path triggers opportunity attack!")
        ...
        print(f"[CHARGE] Opportunity attack didn't stop charge - continuing!")
    ...
    new_facing = calculate_facing_direction(best_destination[0], best_destination[1], enemy_q, enemy_r)
    ...
    if result.is_critical_fail:
        ...
        damage_breakdown = apply_damage(unit, enemy_unit, result.damage, result.is_critical, result.is_overpower)
    ...
    # Deduct full AP cost
    unit.current_action_points -= CHARGE_AP_COST
    print(f"[CHARGE] AP cost: {CHARGE_AP_COST}. Remaining: {unit.current_action_points}/{unit.max_action_points}")
