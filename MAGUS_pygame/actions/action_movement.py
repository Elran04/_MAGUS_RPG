...
def compute_reachable(state: GameState) -> set[tuple[int, int]]:
    ...
    state.enemy_zone_hexes = compute_reach_hexes(
        eq, er, enemy_unit.facing, enemy_unit.size_category
    )
...
def apply_move_if_valid(state: GameState, q: int, r: int) -> bool:
    ...
    if state.active_unit.current_action_points < ap_cost:
        print(
            f"{state.active_unit.name} doesn't have enough AP to move {distance} hexes! (Need {ap_cost}, have {state.active_unit.current_action_points})"
        )
        return False
    ...
    print(
        f"[MOVEMENT] Triggers opportunity: {triggers_opportunity}, intersection at index: {intersection_index}"
    )
    ...
    if triggers_opportunity and intersection_index is not None:
        ...
        attacked, opp_msg = handle_opportunity_attack(
            state, state.active_unit, triggers_opportunity=True
        )
        ...
        if attacked and check_defeat(state):
            ...
            print(
                f"AP deducted for {actual_distance} hex(es): {actual_ap_cost}. Remaining: {state.active_unit.current_action_points}/{state.active_unit.max_action_points}"
            )
            return True
        ...
        if attacked:
            print(
                f"[MOVEMENT] Opportunity attack occurred - movement interrupted at {intersection_hex}"
            )
            # Deduct AP only for distance to intersection
            state.active_unit.current_action_points -= actual_ap_cost
            print(
                f"{state.active_unit.name} moved {actual_distance} hex(es) to {intersection_hex} (cost: {actual_ap_cost} AP). AP remaining: {state.active_unit.current_action_points}/{state.active_unit.max_action_points}"
            )
    ...
    print(
        f"{state.active_unit.name} moved {distance} hex(es) (cost: {ap_cost} AP). AP remaining: {state.active_unit.current_action_points}/{state.active_unit.max_action_points}"
    )
