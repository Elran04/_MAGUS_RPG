...
class UnitInfoPopup:
    ...
    def _draw_tabs(self, screen: pygame.Surface, x_left: int, x_right: int, y: int) -> int:
        ...
        for i, (tab_id, tab_label) in enumerate(tabs):
            ...
            tab_rect = pygame.Rect(tab_x, y, tab_width - 5, 35)
            ...
            # Tab label
            label_text = self.small_font.render(tab_label, True, UI_TEXT)
    ...
    def _draw_armor_tab(self, screen: pygame.Surface, x_left: int, x_right: int, y: int, popup_x: int, popup_width: int) -> int:
        ...
        # Header
        header = self.header_font.render("Armor Coverage", True, (200, 200, 100))
        screen.blit(header, (x_left, y))
        y += 35
        
        if not self.silhouette_image:
            # Fallback if image not loaded
            no_image = self.text_font.render("Silhouette image not available", True, (150, 150, 150))
        ...
        # Scale if needed to fit in popup (max 300x400)
        max_width = 300
        max_height = 380
        ...
        # TODO: Draw colored overlays on body parts that are armored
        # This will be based on unit.armor data when implemented
        # For now, just show the silhouette
        
        y += new_height + 10
        
        # Placeholder text for armor pieces
        armor_text = self.small_font.render("(Armor data not yet implemented)", True, (150, 150, 150))
