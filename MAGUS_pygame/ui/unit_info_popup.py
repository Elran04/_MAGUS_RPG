...
class UnitInfoPopup:
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        # Popup background
        pygame.draw.rect(screen, self.style.bg_color, self.popup_rect, border_radius=self.style.border_radius)
        pygame.draw.rect(screen, UI_BORDER, self.popup_rect, width=3, border_radius=self.style.border_radius)
        ...
        # Close instruction
        close_surface = self.style.small_font.render("(Click outside window to close)", True, (150, 150, 150))
    ...
    def _draw_armor_tab(self, screen: pygame.Surface, x_left: int, x_right: int, y: int, popup_x: int, popup_width: int) -> int:
        """Draw armor tab content with humanoid silhouette. Returns new y offset."""
        # Header
        header = self.style.header_font.render("Armor Coverage", True, self.style.color_header_armor)
        screen.blit(header, (x_left, y))
        y += 35

        if not self.silhouette_image:
            # Fallback if image not loaded
            no_image = self.style.text_font.render("Silhouette image not available", True, (150, 150, 150))
        ...
        if scale < 1.0:
            ...
            scaled_image = pygame.transform.smoothscale(self.silhouette_image, (new_width, new_height))
        else:
        ...
        armor_text = self.style.small_font.render("(Armor data not yet implemented)", True, (150, 150, 150))
    ...
    def _draw_health_section(self, screen: pygame.Surface, x: int, y: int) -> int:
        ...
        # Background bar
        pygame.draw.rect(screen, self.style.bar_bg, (bar_x, bar_y, bar_width, bar_height), border_radius=5)
        
        # Fill bar (blue for FP)
        if fp_max > 0:
            fill_width = int((fp_current / fp_max) * bar_width)
            pygame.draw.rect(screen, self.style.fp_fill, (bar_x, bar_y, fill_width, bar_height), border_radius=5)
        ...
        # ÉP bar
        pygame.draw.rect(screen, self.style.bar_bg, (bar_x, bar_y + self.style.health_ep_bar_delta_y, bar_width, bar_height), border_radius=5)
        
        # Fill bar (red for ÉP)
        if ep_max > 0:
            fill_width = int((ep_current / ep_max) * bar_width)
            pygame.draw.rect(screen, self.style.ep_fill, (bar_x, bar_y + self.style.health_ep_bar_delta_y, fill_width, bar_height), border_radius=5)
        
        pygame.draw.rect(screen, UI_BORDER, (bar_x, bar_y + self.style.health_ep_bar_delta_y, bar_width, bar_height), width=2, border_radius=5)
        y += 35
    ...
    def _draw_attributes(self, screen: pygame.Surface, x: int, y: int) -> int:
        ...
        # Header
        header = self.style.header_font.render("Attributes", True, self.style.color_header_attributes)
        screen.blit(header, (x, y))
        y += 30
    ...
    def _draw_weapon_info(self, screen: pygame.Surface, x_left: int, x_right: int, y: int) -> int:
        ...
        # Header
        header = self.style.header_font.render("Equipped Weapon", True, self.style.color_header_equipment)
        screen.blit(header, (x_left, y))
        y += 30
        ...
        damage_text = self.style.small_font.render(f"Damage: {damage_min}-{damage_max}", True, UI_TEXT)
        ...
        if wield_mode == 'Változó':
            ...
            if wield_info:
                ...
                if can_choose:
                    choice_text = self.style.small_font.render("(Can switch modes)", True, (150, 200, 150))
                    screen.blit(choice_text, (x_left + 30, y))
                    y += 20
                else:
                    forced_text = self.style.small_font.render("(Forced - need more stats)", True, (200, 150, 100))
                ...
                if current_mode == "2-handed" and can_choose and any(bonuses.values()):
                    ...
                    bonus_text = self.style.small_font.render(f"Bonuses: {bonus_str}", True, (100, 255, 255))
    ...
    def _draw_conditions(self, screen: pygame.Surface, x: int, y: int) -> int:
        ...
        # Header
        header = self.style.header_font.render("Conditions", True, self.style.color_header_conditions)
        screen.blit(header, (x, y))
        y += 30
