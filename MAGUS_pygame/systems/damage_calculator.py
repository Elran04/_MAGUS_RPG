...
def calculate_attribute_damage_bonus(unit: Unit) -> int:
    ...
    print(
        f"[DAMAGE CALC] {unit.name} attribute bonuses: {', '.join(bonus_breakdown)} = Total +{total_bonus}"
    )
...
def calculate_damage(unit: Unit, base_damage: int, state: GameState = None) -> dict[str, any]:
    ...
    print(
        f"[DAMAGE CALC] {unit.name}: Base={base_damage}, Attr Bonus={attribute_bonus}, Multiplier=x{charge_multiplier}, Final={final_damage}"
    )
