...
class WeaponsAndShieldsEditor:
    def reset_edit_vars(self):
        """
        Minden edit mező értékét alapértelmezett értékre állítja (nem hoz létre új változókat).
        """
        # Alap mezők
        ...
        # Wield mode
        if 'wield_mode' in self.edit_vars:
            self.edit_vars['wield_mode'].set("Egykezes")
        if 'variable_dual_wield' in self.edit_vars:
            self.edit_vars['variable_dual_wield'].set(0)
        # Checkboxes
        for key in self.CHECKBOX_FIELDS:
            if key in self.edit_vars:
                self.edit_vars[key].set(0)
        # Damage types and bonus attributes
        for typ in self.DAMAGE_TYPES:
            self.damage_type_vars[typ].set(0)
        for attr in self.DAMAGE_BONUS_ATTRS:
            self.damage_bonus_attr_vars[attr].set(0)
        # Kategória mező
        if 'category' in self.edit_vars:
    ...
    def new_item(self):
        ...
        """
        self.selected_idx = None
        # Treeview kijelölés törlése
        if hasattr(self, 'tree'):
            self.tree.selection_remove(self.tree.selection())
        self.reset_edit_vars()
        # Típus mező explicit beállítása, majd callback
        if 'type' in self.edit_vars:
            default_type = self.manager.get_weapon_types()[0]
            self.edit_vars['type'].set(default_type)
            self.update_type_fields(default_type)
        # Fókusz az első mezőre (név)
        if hasattr(self, 'edit_frame_widgets') and self.edit_frame_widgets:
            try:
                self.edit_frame_widgets[0].focus_set()
            except Exception:
                pass
